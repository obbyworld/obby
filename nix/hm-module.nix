# Home Manager module: `programs.obby`.
#
...
#   `inputs.obby.packages.${pkgs.system}.default` overrides).
#
...
in
{
  options.programs.obby = {
    enable = mkEnableOption "Obby — Modern IRC Client for the web, desktop and mobile.";

    package = mkOption {
      type = types.package;
      default =
        if hasAttr "obby" pkgs then
          pkgs.obby
        else
          throw ''
            obby home-manager module: pkgs.obby missing.
            Add this flake's overlay (outputs.overlays.default) to `nixpkgs.overlays`,
            or set `programs.obby.package`, e.g. to
            inputs.obby.packages.${pkgs.stdenv.hostPlatform.system}.default.
          '';
      defaultText = literalExpression "pkgs.obby";
      description = ''
        Obby package.
        Defaults to `pkgs.obby` after merging `outputs.overlays.default` into nixpkgs.
    ...
    viteBuildEnv = mkOption {
      ...
      '';
      description = ''
        Bake-time overrides forwarded into `nix/obby.nix` (every key must start with `VITE_`).
    ...
    autostart = mkEnableOption "Launch Obby when your graphical session starts (XDG autostart). Requires Home Manager's XDG integration: xdg.enable should be true (usually the default).";
  };
  ...
  config = mkIf cfg.enable (mkMerge [
      ...
      assertions = [
        {
          ...
          message = "programs.obby.autostart needs `xdg.enable = true` in Home Manager.";
        }
        {
          assertion = viteKeysOk;
          message = "programs.obby.viteBuildEnv: every key must start with VITE_.";
        }
        {
          assertion = cfg.viteBuildEnv == { } || (cfg.package ? override);
          message = "programs.obby.viteBuildEnv requires a package produced by Obby’s flake/overlay (`callPackage` .override support). Replace `programs.obby.package` or simplify `viteBuildEnv`.";
        }
      ];
    ...
    (mkIf cfg.autostart {
      xdg.configFile."autostart/obby.desktop".text = generators.toINI { } {
        "Desktop Entry" = {
          Type = "Application";
          Name = "Obby";
